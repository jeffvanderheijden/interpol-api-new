...
module.exports = async function completeHandler(req, res) {
    ...
    if (user.role !== "student") {
        return sendOk(res, { points: 0, durationSeconds: 0 });
    }
    ...
    try {
        ...
        const result = await completeChallenge(pool, user.username, challengeId);

        return sendOk(res, result);
    } catch (err) {
        logError("POST /api/challenges/:id/complete", err);
